function[P]=lookbackcall(S,T,M,r,sigma)
    ...
    while(i<=2^(M-1))
        ...
        while(j<=M)
            ...
            min_stock=min(min_stock,stock);%maintains the min value of the path
            j=j+1;
        end
        U(i+1,M+1)=stock-min_stock;%payoff calculated for call option
        i=i+1;
    end
